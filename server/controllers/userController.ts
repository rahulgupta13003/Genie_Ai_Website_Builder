...
const CREDIT_PLANS: Record<
  string,
  { name: string; amountInCents: number; credits: number }
> = {
...
async function addMessage(projectId: string, role: "user" | "assistant", content: string) {
...
export const createUserProject = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const enhancedPrompt =
      (enhanceRes.content[0] as { type: "text"; text: string }).text ||
      initial_prompt;

    await addMessage(
      project.id,
      "assistant",
      `✦ Here's my enhanced brief for your website:\n\n${enhancedPrompt}`
    );
    ...
    if (!cleanCode || !/<!doctype\s+html>/i.test(cleanCode)) {
      await addMessage(
        project.id,
        "assistant",
        "⚠ I couldn't generate the website. Please try again with a different prompt."
      );
    ...
    await addMessage(
      project.id,
      "assistant",
      "✅ Your website is ready! You can preview it and request changes anytime."
    );
  } catch (error: any) {
    await prisma.user
      .update({
...
export const confirmCheckoutPayment = async (req: Request, res: Response) => {
  try {
    const userId = req.userId;
    if (!userId) return res.status(401).json({ message: "Unauthorized" });
