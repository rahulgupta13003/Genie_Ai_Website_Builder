...
async function createConversationEntry(projectId, role, content) {
    return prisma.conversation.create({ data: { role, content, projectId } });
}
async function refundCredits(userId, amount = 2) {
...
export const makeRevision = async (req, res) => {
    ...
    try {
        ...
        try {
            const raw = enhanceRes.content[0].text;
            enhancement = JSON.parse(raw);
        }
        catch {
            // fallback — use original message
        }
        await createConversationEntry(projectId, "assistant", `✦ Enhanced instruction: *"${enhancement.instruction}"*\n` +
        ...
        if (!cleanCode || !/<!doctype\s+html>/i.test(cleanCode)) {
            await createConversationEntry(projectId, "assistant", "⚠ Code generation failed — no valid HTML returned. Please try rephrasing your request.");
        ...
        const versionDescription = summaryRes.content[0].text.trim();
        ...
        await createConversationEntry(projectId, "assistant", `✅ Done! ${versionDescription}`);
        return res.json({
...
export const rollbackToVersion = async (req, res) => {
    try {
        ...
        await createConversationEntry(projectId, "assistant", `↩ Rolled back to: *"${version.description || "previous version"}"*`);
        return res.json({
...
export const getPublishedProjects = async (req, res) => {
    try {
        ...
        return res.json({ projects, total, page: parseInt(page) });
    }
    catch (error) {
