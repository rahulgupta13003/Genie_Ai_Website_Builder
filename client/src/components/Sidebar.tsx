...
import {  BotIcon,  EyeIcon, Loader2Icon, SendIcon, User } from 'lucide-react';
    ...
    SidebarProps) => {
        ...
        const handleRollback = async (versionId: string) => {
            try {
                ...
                toast.error(error?.response?.data?.message || 'Failed to roll back version')
            }
        }
        ...
        const handleRevisions = async (e: React.FormEvent) => {
            ...
            try {
                await api.post(`/api/project/revision/${project.id}`, {message})
                const {data} = await api.get(`/api/user/project/${project.id}`)
                setProject(data.project)
            } catch (error: any) {
                toast.error(error?.response?.data?.message || 'Failed to apply website changes')
            } finally {
      ...
      <div className='flex flex-col h-full'>
        ...
        <div className='flex-1 overflow-y-auto no-scrollbar px-3 flex flex-col gap-4'>
            ...
            .sort((a,b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime()).map((message)=>{
                ...
                if(isMessage){
                    ...
                    return(
                        <div key={ver.id} className=' w-4/5 mx-auto my-2 p-3 bg-gray-800 text-gray-100 shadow flex flex-col gap-2'>
                        <div className='text-xs font-medium'>
                            code Updated <br/> <span className='text-gray text-xs font-normal'>
        ...
        <form onSubmit={handleRevisions} className='m-3 relative'>
            <div className='flex items-center gap-2'>
                <textarea onChange={(e)=> setInput(e.target.value)} value={input} rows={4} placeholder='Describe your website or request changes...' className='flex-1 p-3 rounded-xl resize-none text-sm outline-none ring-gray-700 focus:ring-indigo-500 bg-gray-800 text-gray-100 placeholder-gray-400 transition-all' disabled={isGenerating}/>
