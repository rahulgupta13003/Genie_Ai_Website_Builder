...
const Community = () => {
  ...
  const fetchProjects = async () => {
    try {
      ...
      toast.error(error?.response?.data?.message || 'Failed to fetch published projects')
    } finally {
      setLoading(false)
    }
  }
    ...
    <>
      <div className='px-4 md:px-16 lg:px-24 xl:px-32'>
        {loading ? (
          ...
          <button onClick={() =>navigate('/')} className='text-white px-5 py-2 mt-5 rounded-md bg-indigo-500 hover-indig0-600 active:scale-95 transition-all'>
            Create New Project
          </button>
          </div>
        )}
